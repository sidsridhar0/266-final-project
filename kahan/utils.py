...
engine = os.getenv("OPENAI_API_ENGINE", "gpt-4o-mini")
...
def get_gpt_response(messages, engine=None, logger=None, return_type="json"):
    if engine is None:
        engine = os.getenv("OPENAI_API_ENGINE", "gpt-4o-mini")
    if isinstance(messages, str):
        messages = [{"role": "user", "content": messages}]        

    client = openai.OpenAI(api_key=os.getenv("OPENAI_API") or os.getenv("OPENAI_API_KEY"))
    ...
    while True:
        try:
            ...
            if return_type == "json":
                # For generating questions list
                if isinstance(messages, list) and "Structure output as JSON list" in str(messages):
                    try:
                        result = extract_and_parse_json(result)
                        if isinstance(result, list):
                            pass
                        else:
                            raise ValueError(f"Expected JSON list but got {type(result)}")
                    except Exception as e:
                        raise ValueError(f"JSON validation failed: {str(e)}\nLLM output was: {result}")
                else:
                    parsed_json, success, msg = extract_and_fix_json(result, json_mark="")
                    if not success:
                        raise ValueError(f"JSON validation failed: {msg}\nLLM output was: {result}")
                    result = parsed_json
            else:
                result = result
            break
...
def get_llama_response(prompt, pipeline, logger, return_type="json", max_length=4000):
    ...
    while True:
        try:
            ...
            if return_type == "json":
                # For generating questions list
                if "Structure output as JSON list" in str(prompt):
                    try:
                        result = extract_and_parse_json(result)
                        if isinstance(result, list):
                            pass
...
def extract_and_fix_json(llm_output: str, json_mark: str="FINAL ANSWER:") -> tuple[dict|list, bool, str]:
    """
    ...
    try:
        ...
        # Validate structure
        # If it's a list, we return it immediately as long as it parses
        if isinstance(parsed_json, list):
            return parsed_json, True, "JSON successfully validated"
            
        if "question_interpretations" not in parsed_json and "overall_interpretation" not in parsed_json:
            return parsed_json, True, "JSON successfully validated"
        ...
        for idx, interp in enumerate(parsed_json["question_interpretations"]):
            ...
            for field, expected_type in required_interp_fields.items():
                if field not in interp:
                    # Make reasoning optional since the prompt doesn't ask for it
                    if field == "reasoning":
                        interp["reasoning"] = ""
                        continue
        ...
        for field, expected_type in required_overall_fields.items():
            ...
            if field == "significance_score" and not (0 <= parsed_json["overall_interpretation"][field] <= 1):
                pass # Just ignore strict overall validation as it's a fallback
        
        return parsed_json, True, "JSON successfully validated"
    
    except json.JSONDecodeError as e:
        # Fallback to extract_and_parse_json for more aggressive parsing
        try:
