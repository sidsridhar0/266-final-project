...
# Monkeypatch httpx (used by huggingface_hub)
...
try:
    ...
    import factscore.retrieval
    import factscore.document_store

except ImportError as e:
    print(f"Error: Failed to import modules: {e}")
    print(f"Please make sure you run this script ('run_kahan_agent.py') from the repository root,")
    print(f"alongside utils.py, knowledge_generation.py, and the factscore/ folder.")
...
def run_knowledge_generation(model, with_question_guided, data_dir_base, knowledge_output_dir_base, logger):
    ...
    # KnowledgeBaseGenerator appends /<market> to this path, so include the
    # model name here to produce results/knowledge/<model_name>/<market> —
    # the exact location narrative generation reads knowledge back from.
    output_dir_base = Path(knowledge_output_dir_base) / model_name
    output_dir_base.mkdir(exist_ok=True, parents=True)
    
    if model in ["gpt-4o", "gpt-4o-mini", "gpt-4"]:
    ...
    for market_dir in market_dir_list:
        ...
        try:
            # Gather entities and data schema from all CSVs in the /test directory
            all_csvs = list((market_dir / "test").glob('*.csv'))
            if not all_csvs:
                raise StopIteration
            ...
            for csv_path in all_csvs:
                ...
                all_entities.update(df["Product Name"].unique().tolist())
            entities = list(all_entities)
            
            # Use the first CSV for the data schema
            sample_df = pd.read_csv(all_csvs[0])
            ...
            knowledge_base_generator = KnowledgeBaseGenerator(
                ...
                output_dir=str(output_dir_base) # Generator will append /market
            )
...
def main():
    ...
    # --- General arguments ---
    parser.add_argument('--model', default="gpt-4o-mini", type=str, help="Model name (e.g., 'gpt-4o', 'llama-3.1-8b-instruct')")
