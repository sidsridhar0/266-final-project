...
class NarrationGenerator:
    ...
    def generate_precomputation_code(self, question: str, code_path: str, insight_type: str) -> Dict:
        ...
        if self.prompting == 'cot':
            ...
            5. Handle missing/invalid values in the data. NOTE: DO NOT use `df.fillna(method='ffill')` as it is deprecated and will crash. Use `df.ffill()` instead.
            6. For time-series calculations (e.g., moving averages, previous day comparisons):
            - Market data skips weekends and holidays. DO NOT use exact date math (like `date - pd.Timedelta(days=1)`) to find the previous day. Instead, sort the data by date and use `.shift()`, `.iloc[-1]`, `.iloc[-2]`, etc.
    ...
    def debug_generate_precomputation_code(self, question: str, code_path: str, error: Optional[str] = None) -> Dict:
        ...
        4. Handle potential null/missing values (Use df.ffill() instead of df.fillna(method='ffill'))
    ...
    def interpret_all_entity_insight(self, entity: str, question_with_result_list: List[Dict]) -> Dict:
        ...
        insight = {
            ...
            # 'question': entity_insight_question['question'],
            # 'computation': precomputation_results,
            'interpretation': interpretation_data.get('overall_interpretation', {}).get('summary', ''),
            'significance_score': interpretation_data.get('overall_interpretation', {}).get('significance_score', 0.0),
            'score_reasoning': interpretation_data.get('overall_interpretation', {}).get('reasoning', ''),
    ...
    def generate_all_computation_code(self):
        ...
        for question in questions:
            ...
            if not pass_test:
                ...
                with open(self.knowledge_dir / 'code_generation_count.txt', 'w') as f:
                    f.write(f"entity: {entity}\tretry_after_generate_counts: {retry_after_generate_counts}\tstatus: fail\n")
                raise RuntimeError(f"Fail to generate {precomputation_code_path}...")
